...
// объявление действий с комплекасными числами, итог которого записывется в другой объект
...
std::ostream& Complex::write(std::ostream& outstream) const {
    return outstream << re << (im >= 0 ? add_point : sub_point) << std::fabs(im) << i_point;
}
...
std::istream& Complex::read(std::istream& instream) {
    ...
    char point(0);
    std::string inp = "";
    instream >> real >> point >> inp;
    if ((point == Complex::add_point || point == Complex::sub_point) && (inp.back() == Complex::i_point)) {
        inp.pop_back();
        std::istringstream instr(inp);
        instr >> imaginary;
        re = real;
